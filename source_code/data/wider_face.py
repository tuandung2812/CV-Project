...
def detection_collate(batch):
    ...
    for _, sample in enumerate(batch):
        img, box, label = sample
        imgs.append(img)
        box = torch.from_numpy(np.array(box)).float()
...
if __name__ == '__main__':
    ...
    data_path = '../dataset/wider_face/gt.txt'
    # rgb_mean = (104, 117, 123)  # bgr order
    mean = [0.485, 0.456, 0.406]
    std = [0.229, 0.224, 0.225]

    img_size = (300, 300)
    dataset = WiderFaceDetection(txt_path=data_path,img_dim=img_size, augment= True)

    x,y,labels =  dataset[100]
    # print(x,y)
